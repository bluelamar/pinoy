...
{{if .OpenRoomsOnly}}
  <th>Register</th>
{{else}}
  <th>Guest Name</th><th>Number of Guests</th><th>Extra Guests</th><th>Extra Rate per Person</th><th>Duration</th>
  <th>Cost</th><th>Guest Check in time</th><th>Guest Check out time</th><th>Check Out</th><th>Purchase Items</th>
{{end}}
</tr>
{{range .Rooms}}
<tr>
...
{{else}}
  <td>{{.GuestInfo}}</td><td>{{.NumGuests}}</td><td>{{.NumExtraGuests}}</td><td>{{.ExtraRate}}</td><td>{{.Duration}}</td><td>{{.Cost}}</td><td>{{.CheckinTime}}</td><td>{{.CheckoutTime}}</td>
